...
# Backup EBOS Services and Drivers
$windir = $([Environment]::GetFolderPath('Windows'))
function Backup-EBOSServices {
$filePath = "$([Environment]::GetFolderPath('Windows'))\EBOSModules\Other\ebosServices.reg"
...
& "$windir\EBOSModules\Scripts\settingsPages.cmd" /hide account
...
function Disable-CoreIsolation {
    & ".\EBOSModules\Scripts\ScriptWrappers\ConfigVBS.ps1" -DisableAllVBS
}
...
function Disable-Mitigations {
    Start-Process -FilePath "EBOSDesktop\7. Security\Mitigations\Disable All Mitigations.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
}
...
# Set EBOS Default Profile Pictures
...
function Set-PowerSettings {
    ...
    if ($DisablePowerSaving) {
        Start-Process -FilePath "EBOSDesktop\3. General Configuration\Power-saving\Disable Power-saving.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }

    if ($DisableHibernation) {
        Start-Process -FilePath "EBOSDesktop\3. General Configuration\Hibernation\Disable Hibernation (default).cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }
