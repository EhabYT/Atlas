...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\EBOSModules\Packages",
...
$safeModePackageList = "$sys32\safeModePackagesToInstall.ebosmodule"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgArgs = "/c title Finalizing Installation - EBOS & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the EBOS team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to EBOS." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to EBOS." -ForegroundColor Red
...
Would you like EBOS to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the EBOS folder.
"@

	if ((Read-MessageBox -Title "EBOS - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
# https://github.com/EBOS/EBOS/issues/1103
function MakeRepairSource {
	$version = '38655.38527.65535.65535'
	$srcPath = "%SystemRoot%\EBOSModules\Packages\WinSxS"
