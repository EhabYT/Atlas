if (Test-Path -Path "C:\Program Files\EBOS Toolbox\EBOSToolbox.exe") {
    Write-Host "EBOS Toolbox is already installed.";
    ...
    try {
        ...
        & curl.exe -LSs "https://github.com/EBOS/ebos-toolbox/releases/latest/download/EBOSToolbox-Setup.exe" -o "$tempDirectory\toolbox.exe"
        if (!$?) {
