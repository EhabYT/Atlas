...
set "servicesPath=%windir%\EBOSDesktop\6. Advanced Configuration\Services"
if not exist "%servicesPath%" (
	echo Services in EBOS folder not found, can't continue.
	if "%*"=="" pause
	exit /b 1
)
...
echo This will reset the configuration of services in the EBOS folder.
...
:main
%title% "Enabling services in the EBOS folder... This might take a while."
for /f "usebackq tokens=*" %%a in (`dir /b /s "%windir%\EBOSDesktop\6. Advanced Configuration\Services" ^| find "(default)"`) do (
	call :run "%%a"
	start /min /high /wait cmd /c "%%a" /silent
)

set "ebosOther=%windir%\EBOSModules\Other"
set "winServices=%ebosOther%\winServices.reg"
set "ebosServices=%ebosOther%\ebosServices.reg"
if exist "%winServices%" (
	if exist "%ebosServices%" call :fullRestore
)
...
echo 2) Restore a full services backup of the default EBOS services
echo 3) Nothing
echo]
choice /c:123 /n /m "Choose a number: [1/2/3] "
if "%errorlevel%"=="1" reg import "%winServices%" > nul
if "%errorlevel%"=="2" reg import "%ebosServices%" > nul
exit /b
